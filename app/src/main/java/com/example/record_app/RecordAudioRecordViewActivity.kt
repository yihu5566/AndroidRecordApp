...
class RecordAudioRecordViewActivity : AppCompatActivity(), View.OnClickListener {
    ...
    public override fun onStart() {
        super.onStart()
        //        if (list.size() > 0) {
        //            event = new RecordProgressEvent(finishFile.getName(), finishFile.getAbsolutePath(), currentProgress);
        //        } else {
        //            event = new RecordProgressEvent(myRecAudioFile.getName(), myRecAudioFile.getAbsolutePath(), currentProgress);
        //        }
        //        EventBus.getDefault().post(new RecordUploadFinishEvent(event, objectname));
    }
    ...
    fun getInputCollection(list: ArrayList<String>) {
        // 创建音频文件,合并的文件放这里
        finishFile = File(myRecAudioDir, SUFFIX)
